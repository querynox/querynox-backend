...
const chatController = {
    ...
    handleChatCombined: async (req, res) => {
        try {
            ...
            if (chatId) {
                ...
                const chatName = await serviceManager.generateChatName(prompt);
            ...
            if (webSearch == "true" || webSearch == true) {
                context += await serviceManager.search([...conversationHistory, { role: 'user', content: prompt }]);
                user.usedWebSearch++;
            }
            if (files && files.length > 0) {
                context += await serviceManager.getContextFromFiles(prompt, files);
                user.usedFileRag++;
            }
            ...
            const { content, ...rest } = await serviceManager.generateResponse(model, messages, systemPrompt);
    ...
    getAvailableModels: async (req, res) => {
        try {
            res.status(200).json(serviceManager.getAvailableModels());
        } catch (error) {
            res.status(500).json({ error: 'An internal server error occurred.' });
        }
    },

    // New endpoint for service health check
